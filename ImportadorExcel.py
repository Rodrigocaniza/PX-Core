...
def _periodo_de_fecha_texto(fecha_texto):
    try:
        return datetime.strptime(fecha_texto, "%d-%m-%Y").strftime("%m-%Y")
    except (TypeError, ValueError):
        return None


def crear_respaldo_eliminacion(periodo):
    carpeta = BASE_DIR / "Respaldos"
    carpeta.mkdir(parents=True, exist_ok=True)
    marca = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ruta = carpeta / f"Antes_eliminar_{periodo}_{marca}.zip"
...
def _linea_pertenece_periodo(nombre, linea, periodo):
    try:
        if nombre == "Movimientos":
            datos = Movimientos.separar_movimiento(linea)
            return datos is not None and _periodo_de_fecha_texto(datos["fecha"]) == periodo
        if nombre == "Adicionales":
            datos = Movimientos.separar_adicional(linea)
            return datos is not None and _periodo_de_fecha_texto(datos["fecha"]) == periodo
        ...
        if nombre == "Retiros":
            datos = Socios.separar_retiro(linea)
            return datos is not None and _periodo_de_fecha_texto(datos["fecha"]) == periodo
        if nombre == "Liquidaciones":
            datos = Liquidaciones.separar_liquidacion(linea)
            return datos is not None and datos["periodo"] == periodo
        if nombre == "Prestamos":
            datos = Movimientos.separar_prestamo(linea)
            return datos is not None and _periodo_de_fecha_texto(datos["fecha"]) == periodo
    except Exception:
...
def analizar_periodo_cargado(periodo):
    ...
    for nombre in (
        ...
        lineas = leer_datos(RUTAS[nombre])
        coincidentes = [
            linea for linea in lineas
            if _linea_pertenece_periodo(nombre, linea, periodo)
        ]
        conteos[nombre] = len(coincidentes)
        detalles[nombre] = coincidentes
    return periodo, conteos, detalles


def eliminar_periodo_cargado(periodo):
    periodo, conteos, detalles = analizar_periodo_cargado(periodo)
    if sum(conteos.values()) == 0:
        raise ValueError(f"No se encontraron datos del período {periodo}.")

    respaldo = crear_respaldo_eliminacion(periodo)
    originales = {}
    try:
        for nombre in (
            "Movimientos",
            ...
            originales[nombre] = leer_datos(RUTAS[nombre])
            borrar = set(detalles[nombre])
            nuevas = [linea for linea in originales[nombre] if linea not in borrar]
            if nuevas != originales[nombre]:
                guardar_datos(RUTAS[nombre], nuevas)
    except Exception:
        for nombre, contenido in originales.items():
            guardar_datos(RUTAS[nombre], contenido)
        raise

    return respaldo, conteos
...
def abrir_importador(ventana_padre):
    ...
    def eliminar_periodo():
        dialogo = ctk.CTkInputDialog(
            text="Ingresá el período a eliminar (MM-AAAA):",
            title="Eliminar período importado",
        )
        valor = dialogo.get_input()
        if valor is None:
            return
        try:
            periodo, conteos, _ = analizar_periodo_cargado(valor)
        except ValueError as exc:
            messagebox.showerror(
                "Período inválido",
                str(exc),
        ...
        if total == 0:
            messagebox.showinfo(
                "Sin datos",
                f"No se encontraron datos del período {periodo}.",
                parent=ventana,
            )
            return
        ...
        etiquetas = {
            ...
            "Prestamos": "Préstamos",
            "Cuotas": "Cuotas",
            "Retiros": "Movimientos personales",
            "Liquidaciones": "Liquidaciones",
        }
        resumen = "\n".join(
            f"{etiquetas[nombre]}: {conteos[nombre]}"
            for nombre in etiquetas
        )
        confirmacion = ctk.CTkInputDialog(
            text=(
                f"Se eliminarán TODOS los datos de {periodo}.\n\n"
                f"{resumen}\n\n"
                f"Total: {total} registros.\n\n"
                f"Escribí ELIMINAR {periodo} para confirmar:"
            ),
            title="Confirmación obligatoria",
        ).get_input()
        if confirmacion != f"ELIMINAR {periodo}":
            messagebox.showwarning(
                "Cancelado",
                "La confirmación no coincide. No se eliminó ningún dato.",
                parent=ventana,
            )
            return

        try:
            respaldo, eliminados = eliminar_periodo_cargado(periodo)
        except Exception as exc:
            messagebox.showerror(
                "No se pudo eliminar",
                str(exc),
        ...
        messagebox.showinfo(
            "Período eliminado",
            (
                f"Se eliminaron {sum(eliminados.values())} registros "
        ...
        mostrar_texto(
            f"PERÍODO ELIMINADO: {periodo}\n\n"
            + resumen
            + f"\n\nTOTAL ELIMINADO: {sum(eliminados.values())}"
            + f"\nRESPALDO: {respaldo}"
        )
