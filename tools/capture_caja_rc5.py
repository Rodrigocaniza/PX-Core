"""Smoke visual reproducible de BC Caja rc.5 a 1366x768."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bc-caja-rc5-") as data_dir:
        ...
        def smoke(root):
            ...
            if missing:
                raise RuntimeError(f"faltan controles rc.5: {missing}")
            panels = [w.master for w in labels if str(w.cget("text")) in (
                "CLIENTE Y COMPROBANTE", "DETALLE DE VENTA", "PAGO"
            )]
            print("BC_CAJA_RC5_PANELS " + " ".join(
            ...
            print("BC_CAJA_RC5_VISUAL_SMOKE_OK resolution=1366x768 clear=sale+outflow observations=multiline")
            root.destroy()
