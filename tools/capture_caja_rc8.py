"""Smoke visual reproducible de BC Caja rc.8 a 1366x768."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bc-caja-rc8-") as data_dir:
        ...
        def smoke(root):
            ...
            if missing:
                raise RuntimeError(f"faltan controles rc.8: {missing}")
            ...
            product_type.insert(0, "Cortesía RC8")
            for price in prices:
                price.delete(0, "end")
            ...
            print("BC_CAJA_RC8_VISUAL_SMOKE_OK resolution=1366x768 clear=sale+outflow observations=combined-height discounts=no-cost layout=two-columns rows=5")
            root.destroy()
