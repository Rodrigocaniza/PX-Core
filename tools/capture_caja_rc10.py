"""Smoke visual reproducible de BC Caja rc.10 a 1366x768."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bc-caja-rc10-") as data_dir:
        ...
        def smoke(root):
            ...
            if missing:
                raise RuntimeError(f"faltan controles rc.10: {missing}")
            ...
            product_type.insert(0, "Cortesía RC10")
            for price in prices:
                price.delete(0, "end")
            ...
            expected = {"producto": .45, "codigo": .11, "tipo": .12, "armazon": .11, "cristal": .11, "subtotal": .10}
            ...
            print("BC_CAJA_RC10_PANELS " + " ".join(
            ...
            print("BC_CAJA_RC10_VISUAL_SMOKE_OK resolution=1366x768 clear=sale+outflow observations=combined-height discounts=no-cost layout=two-columns rows=5")
            root.destroy()
