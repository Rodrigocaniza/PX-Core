"""Smoke visual reproducible de BC Caja rc.7 a 1366x768."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bc-caja-rc7-") as data_dir:
        ...
        def smoke(root):
            ...
            if missing:
                raise RuntimeError(f"faltan controles rc.7: {missing}")
            ...
            print("BC_CAJA_RC7_VISUAL_SMOKE_OK resolution=1366x768 clear=sale+outflow observations=combined-height discounts=no-cost layout=two-columns rows=5")
            root.destroy()
