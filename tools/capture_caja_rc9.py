"""Smoke visual reproducible de BC Caja rc.9 a 1366x768."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bc-caja-rc9-") as data_dir:
        ...
        def smoke(root):
            ...
            if missing:
                raise RuntimeError(f"faltan controles rc.9: {missing}")
            ...
            product_type.insert(0, "Cortesía RC9")
            for price in prices:
                price.delete(0, "end")
            ...
            print("BC_CAJA_RC9_VISUAL_SMOKE_OK resolution=1366x768 clear=sale+outflow observations=combined-height discounts=no-cost layout=two-columns rows=5")
            root.destroy()
