...
def _clave_orden_cuota(detalle):
    ...
    return (
        ...
        str(detalle.get("legajo", "")).casefold(),
    )
...
def listar_detalles(planilla_id=None):
    ...
    # Corrige también registros históricos: una cuota finalizada no puede
    # conservar saldo pendiente.
    corregidos = []
    for detalle in registros:
        detalle = dict(detalle)
        if es_ultima_cuota(detalle.get("numero_cuota")):
...
def calcular_saldo_pendiente(planilla_id):
    """Suma el saldo que queda después de cobrar la cuota de la planilla."""
    if not obtener_planilla(planilla_id):
        raise ValueError("La planilla no existe.")
    return sum(max(int(x.get("saldo_pendiente", 0)), 0) for x in listar_detalles(planilla_id))


def planillas_mas_recientes(config_id=None):
    """Devuelve una sola planilla vigente por configuración y categoría.

    Evita sumar saldos históricos varias veces: cada planilla mensual ya contiene
    el saldo remanente proveniente del mes anterior.
    """
    vigentes = {}
    for planilla in listar_planillas(config_id=config_id):
        clave = (planilla["config_id"], planilla["categoria"].casefold())
        periodo_clave = datetime.strptime(planilla["periodo"], "%m-%Y")
        actual = vigentes.get(clave)
        if actual is None or periodo_clave > actual[0]:
            vigentes[clave] = (periodo_clave, planilla)
    return [valor[1] for valor in vigentes.values()]


def resumen_dashboard(config_id=None):
    """Calcula indicadores sin duplicar saldos entre meses históricos."""
    todas = listar_planillas(config_id=config_id)
    vigentes = planillas_mas_recientes(config_id=config_id)
    detalles_vigentes = [
        detalle
        for planilla in vigentes
        for detalle in listar_detalles(planilla["id"])
    ]
    return {
        "planillas": len(todas),
        "registros": len(detalles_vigentes),
        "cuotas_mes": sum(int(x["monto_cuota"]) for x in detalles_vigentes),
        "saldo_pendiente": sum(max(int(x["saldo_pendiente"]), 0) for x in detalles_vigentes),
        "pendientes": sum(1 for x in detalles_vigentes if int(x["saldo_pendiente"]) > 0),
    }
...
def es_ultima_cuota(valor):
    """Indica si la cuota está finalizada, por ejemplo 6/6 o 10/10."""
    try:
        texto = normalizar_numero_cuota(valor)
        if "/" not in texto:
            return False
...
def guardar_detalle(datos):
    ...
    if es_ultima_cuota(numero_cuota):
        saldo_pendiente = 0
    else:
        saldo_pendiente = (
...
def generar_mes_siguiente(planilla_id):
    ...
    for detalle in detalles_origen:
        ...
        monto_configurado = int(detalle["monto_cuota"])

        if "/" in cuota_siguiente:
            cuota_actual, cuota_total = map(int, cuota_siguiente.split("/"))
            es_ultima_cuota = cuota_actual == cuota_total
        else:
            es_ultima_cuota = False

        if es_ultima_cuota or saldo_anterior <= monto_configurado:
            monto_siguiente = saldo_anterior
            saldo_nuevo = 0
        else:
            monto_siguiente = monto_configurado
            saldo_nuevo = saldo_anterior - monto_siguiente
...
def calcular_totales(planilla_id, descuento_manual_override=None):
    ...
    if tipo == "SOLO_PRIMERA_CUOTA":
        # El descuento se aplica una sola vez, en la primera cuota,
        # pero se calcula sobre el total original de la compra.
        base = sum(
            x["total_compra"] for x in detalles
    ...
    return {
        ...
        "saldo_pendiente": sum(max(int(x["saldo_pendiente"]), 0) for x in detalles),
        "pendientes": sum(1 for x in detalles if int(x["saldo_pendiente"]) > 0),
    }
...
def _grupo_letras(numero):
    ...
    partes = []
    if numero >= 100:
        partes.append(CENTENAS[numero // 100])
        numero %= 100
    ...
    if numero < 30:
        ...
        partes.append(
            DECENAS[decena] + (f" y {UNIDADES[unidad]}" if unidad else "")
        )
    return " ".join(partes)


def _apocopar_uno(texto):
...
def numero_en_letras(numero):
    """Convierte enteros de 0 a 999.999.999 a texto en español."""
    valor = int(numero)
    if valor < 0 or valor >= 1_000_000_000:
        raise ValueError("El importe debe estar entre 0 y 999.999.999.")
    ...
    if millones:
        if millones == 1:
            ...
            partes.append(f"{_apocopar_uno(_grupo_letras(millones))} millones")

    if miles:
        if miles == 1:
            partes.append("mil")
        else:
            partes.append(f"{_apocopar_uno(_grupo_letras(miles))} mil")
...
def exportar_excel(planilla_id, ruta_destino):
    ...
    ws.cell(fila, 1, f"Son guaraníes: {numero_en_letras(totales['total'])} guaraníes.")
...
def exportar_pdf(planilla_id, ruta_destino):
    ...
    for ruta_normal, ruta_negrita in rutas_arial:
        if ruta_normal.exists() and ruta_negrita.exists():
            try:
                pdfmetrics.registerFont(TTFont("ArialPX", str(ruta_normal)))
                pdfmetrics.registerFont(TTFont("ArialPX-Bold", str(ruta_negrita)))
                fuente_normal = "ArialPX"
                fuente_negrita = "ArialPX-Bold"
                break
            except Exception:
                pass
    ...
    mes, anio = map(int, planilla["periodo"].split("-"))
    texto_liquidacion = (
    ...
    encabezados = [
        ...
        "Total compra", "Monto cuota", "Saldo",
    ]
    datos = [encabezados]
    for detalle in detalles:
        datos.append([
            ...
            f"{int(detalle['total_compra']):,}".replace(",", "."),
            f"{int(detalle['monto_cuota']):,}".replace(",", "."),
    ...
    resumen = [
        ["", "", "", "Subtotal", "", f"{totales['subtotal']:,}".replace(",", ".")],
    ]
    if totales["descuento"]:
        resumen.append([
            "", "", "", "Descuento", "",
            f"{totales['descuento']:,}".replace(",", "."),
        ])
    resumen.append([
        "", "", "", "Total", "",
    ...
    estilo_resumen.extend([
        ...
        ("ALIGN", (5, 0), (5, -1), "RIGHT"),
        ("FONTSIZE", (3, 0), (5, -1), 8),
        ("TOPPADDING", (3, 0), (5, -1), 2),
        ("BOTTOMPADDING", (3, 0), (5, -1), 2),
        ("BACKGROUND", (3, -1), (5, -1), colors.HexColor("#EAF0F7")),
    ])
    tabla_resumen.setStyle(TableStyle(estilo_resumen))
