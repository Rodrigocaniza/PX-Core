...
class PanelAsociaciones(ctk.CTkFrame):
    ...
    def refrescar_planillas(self):
        ...
        # El saldo general usa solo la última planilla de cada categoría.
        # De lo contrario se sumarían nuevamente saldos ya trasladados a meses posteriores.
        resumen = Asociaciones.resumen_dashboard(config_id=self.config_id)
        self.lbl_saldo_general.configure(
            text=f"Saldo pendiente vigente: {_monto(resumen['saldo_pendiente'])}"
        )
        self.lbl_dash_planillas.configure(text=str(resumen["planillas"]))
        self.lbl_dash_registros.configure(text=str(resumen["registros"]))
        self.lbl_dash_subtotal.configure(text=_monto(resumen["cuotas_mes"]))
    ...
    def nuevo_detalle(self):
        ...
        for campo in (
            ...
            self.d_total, self.d_monto, self.d_saldo,
        ):
            campo.delete(0, "end")
    ...
    def seleccionar_detalle(self, _evento=None):
        ...
        for campo, valor in zip(
            (self.d_legajo, self.d_nombre, self.d_cuota, self.d_total, self.d_monto, self.d_saldo),
