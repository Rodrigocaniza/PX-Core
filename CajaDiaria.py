...
VERSION_APLICACION = "1.0.0-rc.32"
...
def abrir_caja_diaria(ventana_padre, controller=None, usar_ventana_raiz=False):
    ...
    for indice, (clave, etiqueta, ancho) in enumerate(controles_cabecera):
        ...
        # perder la escala de cabecera que ajustó RC18 para el ViewSonic 24".
        destacado = clave == "caja_inicial"
        ctk.CTkLabel(
            cabecera, text=etiqueta,
            text_color=color_azul if destacado else COLOR_TEXTO_SUAVE,
            font=ctk.CTkFont(size=fuente_chrome_cabecera + (1 if destacado else 0),
                             weight="bold"),
        ).grid(
    ...
    def actualizar_estado(cash_day):
        ...
        # `estado_dia` sigue siendo la única que traduce el estado (RC29); acá
        # sólo se le suma la hora que puso el sistema al abrir.
        estado_caja.configure(
            text=f"Estado: {estado_dia(cash_day)}"
                 + (sufijo_hora_apertura(cash_day) if abierta else ""),
    ...
    boton_abrir = ctk.CTkButton(
        ...
        width=104 if compacta else 132, height=alto_boton_estado, corner_radius=4,
        fg_color=color_azul, hover_color="#1D65C5",
        font=ctk.CTkFont(size=fuente_estado, weight="bold"),
    )
