...
Contrato del slice: Pedidos responde "qué requiere atención ahora", la entrada
nunca es una hoja vacía, hay como máximo tres acciones principales y "Corregir
estado" no acepta un estado libre.
"""
...
class PedidosUiContractTests(unittest.TestCase):
    ...
    def test_schema_is_unchanged_at_014(self):
        migraciones = sorted(Path("modulos/caja_diaria/infrastructure/migrations").glob("*.sql"))
        self.assertEqual(len(migraciones), 14)
        self.assertEqual(migraciones[-1].name, "014_order_status_revisions.sql")
