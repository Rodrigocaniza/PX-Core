...
def test_rc24_advances_schema_once_to_019():
    """Cada release avanza el esquema una sola vez; RC24 aporta 019."""
    migrations = sorted(Path("modulos/caja_diaria/infrastructure/migrations").glob("*.sql"))
    assert len(migrations) == 19
    assert migrations[-2].name == "018_branch_binding.sql"
    assert migrations[-1].name == "019_reception_issues.sql"
