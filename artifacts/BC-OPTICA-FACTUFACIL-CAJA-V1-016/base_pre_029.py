# -*- coding: utf-8 -*-
"""Arma una base local en el estado 028, con ventas, para probar el upgrade.

No es la base de la Optica y no puede serlo. Es una base con la misma forma:
migrada hasta la 028, con dias de caja, ventas, gastos y una anulada, para que
aplicar la 029 encima signifique algo.
"""
import shutil
import sys
import tempfile
...
# La 029 se esconde: asi la base nace en el estado anterior, que es el que hay
...
for sql in sorted(MIGR.glob("*.sql")):
    if not sql.name.startswith("029"):
...
print("  factufacil? :", [r[0] for r in con.execute(
    "SELECT name FROM sqlite_master WHERE name LIKE 'factufacil%'")] or "ninguna tabla")
con.close()
